...
import { Content, Header, Hint, NumberOfResults, Position, SearchBox, Scrollable, Wrapper, Close, LoadMore } from './styled'
...
export class MainSearch extends PureComponent {
  ...
  componentDidMount () {
    const { bp } = this.props
    algoliaService.init({
      hitsPerPage: bp('lg') ? searchHitsPerPage.desktop : searchHitsPerPage.mobile
    })
  }
  ...
  render () {
    ...
    const { isFetching, isFetchingPage, searchString, results, count } = this.state
    return (
      <Position>
            ...
            <Header>
              <SearchBox>
                <FormattedMessage id='header.search.placeholder'>{
                  placeholderMsj => <input placeholder={placeholderMsj} value={searchString} onChange={this.handleSearchChange} />
                }</FormattedMessage>
