...
export class ApplePayButton extends React.PureComponent {
  ...
  buildTotal(lineItems, taxValue) {
    const totalLineItems = this.buildLineItems(lineItems, taxValue).reduce(
      (a, lineItem) => a + (parseFloat(lineItem.amount) || 0),
      0
    )
  ...
  buildLineItems(lineItems, taxValue) {
    const includedTaxLabel = this.taxesIncludedInPrice() ? '(Incl GST) ' : ''
    const lineItemsObj = lineItems.map((lineItem) => ({
      type: 'final',
    ...
    const lineItemAdjustments = this.props.order.lineItemAdjustments.map(
      (lineItemAdjustment) => ({
        type: 'final',
  ...
  async handleApplePayClicked() {
    ...
    if (order.shipments[0]) {
      orderShippingMethods = order.shipments[0].shippingMethods.map(
        (shippingMethod) => ({
          label: shippingMethod.name,
          detail: `${shippingMethod.name} shipping`,
          identifier: shippingMethod.id,
          amount: order.shipments[0].shippingRates.find(
            (r) => r.shippingMethodId === shippingMethod.id
          ).cost
        })
      )
    }
    ...
    const session =
      window.Stripe &&
      Stripe.applePay.buildSession(paymentRequest, (result, completion) => {
        const orderData = {
          address: {
    ...
    session.onshippingmethodselected = (event) => {
      ...
      const shippingOptionIndex = lineItems.findIndex((item) =>
        item.label.includes('shipping')
      )
      ...
      const total = {
        label: 'Mejuri',
        amount: lineItems
          .reduce((a, lineItem) => a + (parseFloat(lineItem.amount) || 0), 0)
          .toFixed(2)
      }
      this.setState({
        lineItems: lineItems,
    ...
    session.onshippingcontactselected = (event) => {
      ...
      calculateTaxes(
        ...
        (error) => {
          const lineItems = this.buildLineItems(this.props.lineItems, 0)
          const total = this.buildTotal(lineItems, 0)
...
ApplePayButton.defaultProps = {
  trackEvent: (eventName, context) => {
    console.error('ApplePayButton: missing trackEvent property')
  }
}
